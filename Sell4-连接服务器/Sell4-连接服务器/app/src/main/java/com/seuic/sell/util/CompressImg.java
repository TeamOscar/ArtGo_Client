...
public class CompressImg {
    ...
    public static String bitmapToBase64(String picPath) {
        ...
        try {
            if (picPath != null && !picPath.equals("")) {
                ...
                int h = newOpts.outHeight;
                // 现在主流手机比较多是800*480分辨率，所以高和宽我们设置为
                float hh = 800f;//1200f;// 这里设置高度为800f
                float ww = 600f;//1200f;// 这里设置宽度为480f
                // 缩放比。由于是固定比例缩放，只用高或者宽其中一个数据进行计算即可
                int be = 1;// be=1表示不缩放
                if (w > h && w > ww) {// 如果宽度大的话根据宽度固定大小缩放
                    be = (int) (newOpts.outWidth / ww);
   ...
   /* // 最省内存的加载图片方法
    public static Bitmap readBitMap(int resId) {
        ...
        // 获取资源图片
        InputStream is = AppContext.getAppContext().getResources()
   ...
   /* // 设置背景图片
    public static void setBg(Context context, int resId, View view) {
        ...
        Drawable drawable = new BitmapDrawable(AppContext.getAppContext().getResources(), bitmap);
